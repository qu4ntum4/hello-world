...
var unavailable, readFileSync, existsSync, promises, join, resolve, dirname, node_stub_default;
var init_node_stub = __esm({
  "node-stub.js"() {
    ...
    existsSync = () => false;
    promises = { readFile: unavailable };
    join = (...p) => p.join("/");
    resolve = (...p) => p.join("/");
    dirname = (p) => p.split("/").slice(0, -1).join("/");
    node_stub_default = { readFileSync, existsSync, promises, join, resolve, dirname };
  }
});
...
async function writeCredentialsFileAtomic(targetPath, data) {
  const fs = await Promise.resolve().then(() => (init_node_stub(), node_stub_exports));
  const path2 = await Promise.resolve().then(() => (init_node_stub(), node_stub_exports));
